...
Neotro.addCommand({ pattern: 'alexai ?(.*)', desc: 'ai ALEXA chat bot on off command' , fromMe: true, usage: '.alexi on / off' }, (async (message, match) => {
    ...
    if (match[1] == 'on') {
        if (eva_status == 'true') {
            return await message.client.sendMessage(message.jid, '*Already on ☣️*', MessageType.text)
        }
        else {
        ...
        if (eva_status !== 'false') {
            return await message.client.sendMessage(message.jid, '*☣️chat bot already off*', MessageType.text)
        }
        else {
            await heroku.patch(baseURI + '/config-vars', { 
                body: { 
            ...
            await message.client.sendMessage(message.jid, '*☣️successful Disablechat bot*', MessageType.text)
        }
    }
}));
